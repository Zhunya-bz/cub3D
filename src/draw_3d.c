...
/*   Updated: 2021/11/25 15:53:17 by erichell         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_init_dist(t_data *data)
{
	...
	{
		data->p_coord->stepX = 1;
		data->p_coord->sideDistX = (data->p_coord->mapX + 1.0 - \
		data->p_coord->x) * data->p_coord->deltaDistX;
	}
	if (data->p_coord->ray_vecY < 0)
	{
...
void	ft_dist_wall(t_data *data)
{
	...
	if (data->p_coord->side == 0)
		data->p_coord->perpWallDist = (data->p_coord->mapX - data->p_coord->x \
		+ (1 - data->p_coord->stepX) / 2) / data->p_coord->ray_vecX;
	else
		data->p_coord->perpWallDist = (data->p_coord->mapY - data->p_coord->y + \
	(1 - data->p_coord->stepY) / 2) / data->p_coord->ray_vecY;
	if (data->p_coord->perpWallDist <= eps && data->p_coord->perpWallDist \
	> -eps)
...
void	perpwall(t_data *data)
{
	...
	data->p_coord->drawStart = -data->p_draw->texture->line_height / 2 + \
	data->p_info->screen_height / 2;
	if (data->p_coord->drawStart < 0)
		data->p_coord->drawStart = 0;
	data->p_coord->drawEnd = data->p_draw->texture->line_height / 2 + \
	data->p_info->screen_height / 2;
...
void	ft_draw_texture(t_data *data, int x, int y)
{
	...
	data->p_coord->perpWallDist * data->p_coord->ray_vecY;
	else
		data->p_draw->texture->wallx = data->p_coord->x + \
	data->p_coord->perpWallDist * data->p_coord->ray_vecX;
	...
	if (data->p_coord->side == 1 && data->p_coord->ray_vecY < 0)
		data->p_draw->texture->texx = data->p_draw->texture->height \
	- data->p_draw->texture->texx - 1;
	data->p_draw->texture->tex_pos = (data->p_coord->drawStart - \
	...
	{
		if (data->p_coord->ray_vecY > 0 && data->p_coord->mapY > \
		(int)data->p_coord->y)
			texdir = 1;
		if (data->p_coord->ray_vecY < 0 && data->p_coord->mapY < \
		(int)data->p_coord->y)
...
int	ft_draw_3d(t_data *data)
{
	...
	while (x < data->p_info->screen_width)
	{
		...
		if (data->p_coord->side == 1)
			color = color / 2;
		while (i < data->p_coord->drawStart)
			my_pixel_put(data, x, i++, data->p_draw->c_color);
		i = 0;
		if (data->p_coord->drawStart + i < data->p_coord->drawEnd)
			ft_draw_texture(data, x, data->p_coord->drawEnd);
		while (data->p_coord->drawEnd < data->p_info->screen_height)
			my_pixel_put(data, x, data->p_coord->drawEnd++, \
		data->p_draw->f_color);
		x++;
	}
